...
class T3(nn.Module):
    ...
    def prepare_input_embeds(
        ...
        *,
        ...
        if cfg_weight > 0.0:
            text_emb[1::2].zero_()  # CFG uncond rows
    ...
    def inference(
        ...
        *,
        ...
        def _cfg_repeat(tensor: torch.Tensor) -> torch.Tensor:
            if not cfg_enabled:
                return tensor
            return torch.repeat_interleave(tensor, 2, dim=0)

        model_text_tokens = _cfg_repeat(text_tokens)

        if initial_speech_tokens is None:
            initial_speech_tokens = self.hp.start_speech_token * torch.ones(
                (base_batch, 1), dtype=torch.long, device=self.device
            )
        else:
            initial_speech_tokens = torch.atleast_2d(initial_speech_tokens).to(
                dtype=torch.long, device=self.device
            )
            if initial_speech_tokens.size(0) != base_batch:
        ...
        bos_token = torch.full(
            (base_batch, 1), self.hp.start_speech_token, dtype=torch.long, device=device
        )
        ...
        tail_allowance = getattr(self.hp, "alignment_tail_allowance", 11)
